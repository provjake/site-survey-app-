  ...
  const handleExport = async () => {
    ...
    if (photos.length) {
      const photoParagraphs = await Promise.all(
        photos.map(async ({ file, caption }) => {
          const buffer = await file.arrayBuffer();
          const image = Media.addImage(doc, buffer, 400, 300);
          return [new Paragraph(image), new Paragraph(caption || "")];
        })
      );
      doc.addSection({ children: [new Paragraph({ text: "Photos", heading: HeadingLevel.HEADING_2 }), ...photoParagraphs.flat()] });
    }
